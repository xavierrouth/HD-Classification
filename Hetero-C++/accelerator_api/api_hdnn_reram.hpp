...
private:
  ...
  float BER_sb2te [10]={0.35,0.22,0.18,0.13,0.1,0.08,0.05};//type sb2te3/gst467
  float BER_tite2 [10]={0.3, 0.16,0.1, 0.09,0.07,0.06,0.05};//type tite2/gst467
...
public:
  ...
  sim_hdnn_reram(uint32_t dim_f, uint32_t dim_d, uint32_t num_c,uint32_t mlc_level);
  ~sim_hdnn_reram();
  ...
  void program_pcm_bit();
  void readout_pcm_bit();

  
};

sim_hdnn_reram::sim_hdnn_reram(uint32_t dim_f, uint32_t dim_d, uint32_t num_c,uint32_t mlc_level) {
