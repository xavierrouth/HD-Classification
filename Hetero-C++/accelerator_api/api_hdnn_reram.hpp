...
public:
  ...
  //adc related
  int16_t adc_resolution;
  int16_t adc_step;
  int16_t adc_floor;
  int16_t adc_ceil;


    sim_hdnn_reram(uint32_t dim_f, uint32_t dim_d, uint32_t num_c,uint32_t mlc_level, uint32_t adc_reso_cfg,uint32_t adc_step_cfg,uint32_t adc_ceil_cfg,uint32_t adc_floor_cfg,bool reference_m);
    ~sim_hdnn_reram();
  ...
  void init_enc_mat();
  void init_adc(uint32_t adc_resolution_cfg,uint32_t adc_ceil_cfg,uint32_t adc_floor_cfg,uint32_t adc_step_cfg);
  ...
  int16_t adc_quantize(int16_t step,int16_t floor,int16_t ceil,int16_t input);
  ...
  int16_t array_dot_product(std::array<int16_t,ARRA_SZ>& cell_array_1,std::array<int16_t,ARRA_SZ>& cell_array_2);

  
};

sim_hdnn_reram::sim_hdnn_reram(uint32_t dim_f, uint32_t dim_d, uint32_t num_c,uint32_t mlc_level, uint32_t adc_reso_cfg,uint32_t adc_step_cfg,uint32_t adc_ceil_cfg,uint32_t adc_floor_cfg,bool reference_m) {
  ...
  init_buffer();
  init_reram_array();
  init_adc(adc_reso_cfg,adc_ceil_cfg,adc_floor_cfg,adc_step_cfg);
...
void sim_hdnn_reram::init_adc(uint32_t adc_resolution_cfg,uint32_t adc_ceil_cfg,uint32_t adc_floor_cfg,uint32_t adc_step_cfg)
{
  adc_resolution=adc_resolution_cfg;// default(128 cell array) 7bit value, 1 bit sign,SLC
  adc_ceil=adc_ceil_cfg;
  adc_floor=adc_floor_cfg;
  adc_step=adc_step_cfg;
}
...
*result = 0;
  ...
  int physical_cells=this->dim_hv/this->pcm_mlc_level;
  int array_iterations=physical_cells/ARRA_SZ;
  int remain_cells=physical_cells%ARRA_SZ;


for (uint32_t c = 0; c < this->num_class; c++) {
result[c] = 0;
for (uint32_t j = 0; j < array_iterations; j++) {
  std::array<int16_t,ARRA_SZ> cell_array_1;

  
  for (int t = 0; t < ARRA_SZ; t++)
  {
    cell_array_1[t]=encoded_query[j*ARRA_SZ+t];//construct query arr
  }
  

  
  std::array<int16_t,ARRA_SZ> cell_array_2;
  
  for (uint32_t k = 0; k < ARRA_SZ; k++)
  {
    cell_array_2[k]=this->readout_reram_bit_mlc(c, j*ARRA_SZ+k);//contruct partial ref arr
  }
    

  int16_t partial_result=this->array_dot_product(cell_array_1,cell_array_2);
  int16_t quantized_partial_result=this->adc_quantize(this->adc_step,this->adc_floor,this->adc_ceil,partial_result);
result[c]+=quantized_partial_result;
}


for (uint32_t j = 0; j < remain_cells; j++)
{
  result[c] += this->readout_reram_bit_mlc(c, array_iterations*ARRA_SZ+j)*encoded_query[array_iterations*ARRA_SZ+j];//dot product for remaining, not ideal needs to quantize as well
...
int16_t sim_hdnn_reram::array_dot_product(std::array<int16_t,ARRA_SZ>& cell_array_1,std::array<int16_t,ARRA_SZ>& cell_array_2)
{
  int16_t result=std::inner_product(cell_array_1.begin(),cell_array_1.end(),cell_array_2.begin(),0);
return result;
}


int16_t sim_hdnn_reram::adc_quantize(int16_t step,int16_t floor,int16_t ceil,int16_t input)
{
  if (input>ceil)
  {
    return ceil;
