...
def convert_bin(format_, X_train_path='', y_train_path='', X_test_path='', y_test_path=''):
	...
	if format_ == 'csv':
		...
		X_train_out = X_train_path.replace('.pickle', '_nq_trainX.bin')
		y_train_out = X_train_path.replace('.pickle', '_nq_trainY.bin')
		X_test_out = X_train_path.replace('.pickle', '_nq_testX.bin')
		y_test_out = X_train_path.replace('.pickle', '_nq_testY.bin')

        # Disableing quantization
	...
	np.array(X_train_1d).astype(np.single).tofile(X_train_out)
	np.array(y_train_1d).astype('int32').tofile(y_train_out)
	np.array(X_test_1d).astype(np.single).tofile(X_test_out)
...
convert_bin("pickle", "isolet.pickle", "", "", "");
